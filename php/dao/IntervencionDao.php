...
class IntervencionDao
{
    ...
    public function sp_register_detalle($idIntervencion, $productos_json){
        ...
        try {
            ...
            foreach ($productos as $pro) {
                $stm->execute(array(7, '', 0, $idIntervencion, date('Y-m-d H:i:s'), 0, '', '', 0, '', 0, '', '', 0, 0, '', $pro->codigo, str_replace("'", "\'", $pro->descripcion), str_replace("'", "\'", $pro->marca), $pro->cantidad, $pro->precio));
            }
            $pdo->commit();
        } catch (PDOException $e) {
            $msj['msj'] = 'KO';
    ...
    {
        $respuesta = array('msj' => 'KO', 'nombre' => null, 'error' => '');
        try {
            $stm = $this->pdo->prepare("CALL sp_intervencion(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
